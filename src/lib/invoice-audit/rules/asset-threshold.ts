...
const SK_DHM_THRESHOLD = 1_700
const SK_NMA_THRESHOLD = 2_400

export function checkAssetThreshold(
  ...
  if (country === 'cz') {
    ...
    if (amount >= SK_DHM_THRESHOLD * 0.9 && amount < SK_DHM_THRESHOLD && code !== '022') {
      return {
        id: 'asset',
        label: 'Majetkový práh',
        message: `Suma ${amount.toLocaleString('sk-SK')} EUR blízko prahu DHM (1 700 EUR) — zvažte účet 022.`,
        severity: 'warning',
      }
    }
    if (amount >= SK_DHM_THRESHOLD && code !== '022' && code !== '013') {
      return {
        id: 'asset',
        label: 'Majetkový práh',
        message: `Suma nad 1 700 EUR — DHM by mal byť účet 022, navrhnutý ${code}.`,
        severity: 'warning',
      }
    }
    if (amount >= SK_NMA_THRESHOLD && code === '518') {
      return {
        id: 'asset',
        label: 'Majetkový práh',
        message: `Jednorazový nákup nad 2 400 EUR — zvážte účet 013 namiesto 518.`,
        severity: 'warning',
      }
    }
  }
  ...
  return {
    id: 'asset',
    label: 'Majetkový práh',
    message: `Účet ${code} odpovídá částce ${amount.toLocaleString(country === 'sk' ? 'sk-SK' : 'cs-CZ')} ${country === 'sk' ? 'EUR' : 'Kč'}.`,
    severity: 'ok',
  }
}
